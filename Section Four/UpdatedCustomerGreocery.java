...
public class UpdatedCustomerGreocery {
    ...
    public static String checkGroceryList(
        ...
        if (storeAisle[i].equalsIgnoreCase(customerRequest)
                || storeAisle[i].toLowerCase().contains(customerRequest.toLowerCase())) {

            return "Did you mean \"" + storeAisle[i] + "\"?\n"
                    + "Yes, we have " + storeAisle[i]
                    + " in stock at aisle " + (i + 1) + ".";
        }
    }

    return "Sorry, we do not have \"" + customerRequest + "\" in stock.";
}
}
